...
const TRAITS:Array<keyof KitSpec>=['base','patternInk','pattern','collar','sleeves','makerHe','sponsorHe']
...
export function scoreStudioDesign(spec:KitSpec,briefId:KitBriefId,dna:KitSpec[]):StudioMetrics{
  let identity=35
  if(spec.base==='red'||spec.base==='deep') identity+=30
  if(redDetail(spec)) identity+=15
  if(spec.crestKey) identity+=15
  if(spec.sponsorHe==='HAPOEL'||spec.sponsorHe==='THE WORKER'||spec.sponsorHe==='1923') identity+=5
  ...
  const originality=nearest?clamp(35+differences(spec,nearest)*10):78
  ...
  if(briefId==='derby'){
    ...
    if(spec.pattern!=='solid') briefFit+=20
    ...
    else { const best=[...memory].sort((a,b)=>differences(spec,a)-differences(spec,b))[0]!; briefFit=clamp((likeness(spec,best)>=1?50:0)+(differences(spec,best)>=1?50:0)) }
