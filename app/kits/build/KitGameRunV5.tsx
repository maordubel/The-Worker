...
export function KitGameRunV5({ puzzles, seed, cursor = 0 }: { puzzles: KitPuzzle[]; seed: number; cursor?: number }) {
  ...
  function pick(part: KitPart) {
    ...
    if (nextMissing) {
      timer.current = window.setTimeout(() => setActive(nextMissing), 140)
    }
  }
  ...
  return (
    <div className="mx-auto flex min-h-[calc(100dvh-118px)] max-w-6xl flex-col overflow-hidden pb-2">
      <header className="flex shrink-0 items-end justify-between gap-3 border-b-rule border-ink pb-1.5">
        <div>
          <p className="font-body text-[9px] font-black tracking-[.15em] text-red">שער 4 · זיכרון חולצה · {difficultyLabel(puzzle.difficulty)}</p>
        ...
        <div className="text-end">
          ...
          <p className="font-mono tabular-nums text-[9px] text-muted">{Object.keys(placed).length}/8 · {puzzle.optionCount} אפשרויות</p>
        </div>
      </header>
      ...
      <div className="grid min-h-0 flex-1 grid-rows-[minmax(260px,48dvh)_minmax(0,1fr)] gap-2 pt-2 lg:grid-cols-[minmax(300px,430px)_minmax(0,1fr)] lg:grid-rows-1">
        <section className="flex min-h-0 flex-col border-rule border-ink bg-sheet">
          <div className="min-h-0 flex-1 p-1.5">
            <KitEngineShirt spec={spec} className="mx-auto block h-full max-h-[540px] w-full" title={`חולצה ${puzzle.seasonLabel}`} />
          </div>
          <div className="grid shrink-0 grid-cols-3 border-t-hair border-ink">
            <button type="button" onClick={undo} disabled={!history.length} className="min-h-tap border-e-hair border-ink font-body text-[10px] font-extrabold disabled:opacity-30">↶ בטל</button>
            <button type="button" onClick={reset} disabled={!Object.keys(placed).length} className="min-h-tap border-e-hair border-ink font-body text-[10px] font-extrabold disabled:opacity-30">איפוס</button>
            <button type="button" onClick={() => setAutoAdvance((value) => !value)} className={`min-h-tap font-body text-[9px] font-extrabold ${autoAdvance ? 'bg-ink text-paper' : 'bg-paper text-ink'}`}>אוטומטי {autoAdvance ? '✓' : '—'}</button>
          </div>
        </section>
        ...
        <section className="flex min-h-0 flex-col">
          ...
          <div className="mt-2 flex min-h-0 flex-1 flex-col border-rule border-ink bg-paper">
            <div className="flex shrink-0 items-baseline justify-between border-b-hair border-ink px-2 py-1.5">
              ...
              <span className="font-body text-[9px] text-muted">כל אפשרות מוצגת על אותה חולצה</span>
            </div>
          ...
          {hints.length > 0 && <div className="mt-1 shrink-0 border-hair border-ink/35 bg-sheet px-2 py-1 font-body text-[9px] leading-snug">{hints.map((row) => <span key={row.kind} className="me-2">{row.textHe}</span>)}<span className="font-mono text-red">−{hints.length * KIT_HINT_PENALTY}</span></div>}
...
function difficultyLabel(value: KitPuzzle['difficulty']) {
  if (value === 'warmup') return 'חימום'
  if (value === 'expert') return 'מומחה'
  return 'זיכרון'
}
  ...
  return (
    <div className="mx-auto max-w-5xl pb-8">
      <div className="border-rule border-ink bg-red px-4 py-3 text-paper">
        ...
        <p className="mt-1 font-display text-[36px] leading-none">{verdict.right}/8 · {verdict.score} נק׳</p>
      </div>
      <div className="grid grid-cols-2 border-x-rule border-b-rule border-ink bg-paper">
        ...
        <figure className="p-2">
          ...
          {verdict.realSrc ? <img src={verdict.realSrc} alt={`חולצת ${verdict.seasonLabel}`} className="mx-auto h-[min(36vh,300px)] w-full object-contain" /> : <KitAssemblyShirt spec={verdict.answer} historical className="mx-auto h-[min(36vh,300px)] w-full" />}
        </figure>
      </div>
      <div className="mt-2 grid grid-cols-2 gap-1 sm:grid-cols-4">
        {verdict.parts.map((part) => <div key={part.kind} className={`border-hair px-2 py-2 ${part.correct ? 'border-ink bg-ink text-paper' : 'border-red bg-paper text-red'}`}><p className="font-body text-[10px] font-black">{part.correct ? '✓' : '×'} {PART_LABEL[part.kind]}</p><p className="font-mono text-[8px]">{part.points} נק׳</p></div>)}
      </div>
      <div className="mt-2 border-hair border-ink/35 bg-sheet px-3 py-2">
        ...
        <p className="mt-1 font-mono text-[8px] text-muted">מקור: {verdict.sourceTitle}</p>
      </div>
      {!last ? <button type="button" onClick={onNext} className="mt-2 min-h-[50px] w-full border-rule border-ink bg-ink font-display text-[20px] text-paper">לחולצה הבאה ←</button> : <RunDone />}
    </div>
  )
}

function RunDone() {
  return <div className="border-rule border-ink bg-sheet p-6 text-center"><p className="font-display text-[30px]">הסיבוב הושלם</p><a href="/kits" className="mt-3 inline-flex min-h-tap items-center border-rule border-ink bg-red px-5 font-body text-[12px] font-extrabold text-paper">ל־DNA Studio ←</a></div>
}
