          ...
          {saved.length > 0 && (
            <div className="mt-3 border-t-rule border-ink pt-3">
              ...
              <div className="mt-1 grid grid-cols-3 gap-1">
                {saved.slice(0, 6).map((row) => <button key={row.id} type="button" onClick={() => load(row)} className="border-hair border-ink/35 bg-sheet p-1"><KitAssemblyShirt spec={row.spec} className="mx-auto h-20 w-full" /><span className="block font-mono tabular-nums text-[9px] text-ink">{row.metrics.overall}</span></button>)}
              </div>
            </div>
          )}
        </section>
