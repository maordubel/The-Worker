...
function DnaPanel({ unlocked, dnaKeys, onApply, onToggle }: { unlocked: RackKit[]; dnaKeys: string[]; onApply: (row: RackKit) => void; onToggle: (row: RackKit) => void }) {
  if (!unlocked.length) return <a href="/kits/build" className="flex min-h-[120px] items-center justify-center border-rule border-red bg-red px-4 text-center font-body text-[12px] font-extrabold text-paper">{t('kitgame.cta')}</a>
  return (
...
function ColourPanel({ spec, mutate }: { spec: KitSpec; mutate: (patch: Partial<KitSpec>) => void }) {
  return (
    <div className="space-y-3">
      <ColourRow title={t('kit.base')} value={spec.base} onPick={(base) => mutate({ base })} />
      <ColourRow title={t('kit.secondary')} value={spec.patternInk} onPick={(patternInk) => mutate({ patternInk })} />
      <ColourRow title={t('kit.sleeveInk')} value={spec.sleeveInk} onPick={(sleeveInk) => mutate({ sleeveInk })} />
      <ColourRow title={t('kit.collarInk')} value={spec.collarInk} onPick={(collarInk) => mutate({ collarInk })} />
    </div>
  )
}
