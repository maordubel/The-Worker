        ...
        for index, hunk in enumerate(hunks, 1):
            ...
            for line in hunk.splitlines(keepends=True):
                ...
                if line.startswith('-'):
                    ...
                    # Metadata/comments outside the pseudo-hunk are not source context.
                    continue
            old = ''.join(old_lines)
            new = ''.join(new_lines)
            if not old:
            ...
            if count != 1:
                ...
                raise RuntimeError(
                    f'{patch_name}:{target}: hunk {index} expected exactly once, found {count}: {preview}'
                )
...
# Execute the rest of the RC applicator, but skip its original git-apply bootstrap.
