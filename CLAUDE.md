...
## 64 · גשר בין שתי אלפביתות — סירוב הוא תשובה, ניחוש הוא לא (15.9.2026)

מאור ביקש עמדה, מוצא ושנים **לכל** שחקן. המקור היחיד שאפשר היה להגיע אליו הוא לועזי
(worldfootball.net, 73 עונות, 1933/34 → 2025/26, 465 שמות), והארכיון שלנו עברי. כל
המעבר הוא הגשר הזה, וכל קיצור־דרך עליו מייצר **שחקן שגוי עם שם אמיתי**.

1. **התאמה נוצרת רק כשהיא יחידה.** שם עברי נכנס ל-`player-facts.json` רק אם שלד
   העיצורים שלו מתיישר עם **בדיוק אחד** מתוך 465. שניים — הוא הולך ל-`ambiguous`
   עם המועמדים שלו, ולא לקובץ. 8 שמות סורבו כך, והם מודפסים בשמם.
2. **מעבר שם־משפחה בלבד נכתב, נמדד ב-~50% טעויות, ונמחק.** אבי אדרי → Kfir Edri,
   ברונו סוארס → Yaakov Schwartz. לא כוונון — מחיקה, עם הערה בסקריפט שאומרת למה, כדי
   שמי שיחשוב על זה שוב ימצא את התשובה כבר שם.
3. **תביעה אחת לכל שחקן, לשני הכיוונים.** בלי זה סדר האיטרציה מחליט, ו"יעקב כהן"
   לוקח את Yuval Cohen רק כי הוא הגיע ראשון.
4. **כל שורה אומרת איך הגיעה** (`matchedBy`), ו-`FacetSource` מדרג: `squad` > `database`
   > `lineup` > `name`. `lineup` הוא איפה אדם עמד **באותו ערב**, לא עמדת קריירה —
   ולכן מקור מחקרי גובר עליו.
5. **מה שאין — מוצג.** `לא מתועד` הוא דלי אמיתי עם ספירה אמיתית, ו-`unknown`/`ambiguous`
   מודפסים. 332 מתוך 647 שלמים; 315 אומרים "לא יודעים" במקום משהו שנשמע טוב יותר.

מה שנשאר סגור נשאר סגור: ויקיפדיה עברית ואנגלית הן cache-only כאן, ויקיפועל עדיין
חסומה. כלל 11 — מקור חסום מתועד, לא עוקפים אותו. הדרך ל-100% עוברת במאור עצמו
(כלל 18) דרך `players-to-fill.xlsx`, או בייצוא Cargo שדפדפן אנושי מביא.
`docs/09-player-facts.md` מחזיק את המספרים, את הבאגים שנמדדו, ואת השיטה.
